...
export default function ReviewWritePage() {
  ...
  // 💡 Base64 대신 실제 File 객체 배열을 상태로 관리
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. 🚀 Supabase Storage로 이미지 직접 업로드 (Vercel 안 거침)
      ...
      // 2. 텍스트 정보와 Supabase 이미지 URL 배열을 FormData에 담기
      ...
      // 💡 서버(Railway DB API)에서 받을 수 있도록 JSON 문자열로 변환하여 append
      data.append('image_urls', JSON.stringify(imageUrls));

      // 3. 서버 API 호출 (가벼운 텍스트/URL 통신이므로 413 에러 없음)
