...
export default function BannerSliderWithForm({ banners }: { banners: any[] }) {
  ...
  <div className="w-full px-4 md:px-8">
    ...
    {/* 컨테이너: items-stretch를 통해 좌우 높이를 무조건 일치시킴 */}
    <div className="flex flex-col md:flex-row items-stretch gap-6">
      
      {/* 1. 비밀지원금 신청 폼 영역 */}
      <div className="w-full md:w-[420px] bg-white rounded-2xl md:rounded-3xl shadow-2xl p-6 md:p-8 shrink-0 border border-slate-100 flex flex-col justify-between">
        <div>
      ...
      {/* 2. Swiper 배너 슬라이드 영역 (높이가 쪼그라들지 않고 좌측 폼과 세로 크기를 100% 공유) */}
      <div className="w-full md:flex-1 md:self-stretch overflow-hidden rounded-2xl md:rounded-3xl shadow-xl bg-transparent flex flex-col">
        <Swiper
        ...
        >
          {banners.map((banner, index) => (
            <SwiperSlide key={banner.id || index} className="h-full">
              {/* 💡 핵심: aspect 비율을 버리고 h-full을 주어 높이가 쪼그라드는 것을 원천 차단하고 꽉 채움 */}
