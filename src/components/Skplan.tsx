...
export default function SkPlan() {
  // 초기값을 null로 주어 처음에는 어떤 TV도 선택되지 않게 설정
  ...
  // 인터넷 기본 가격 (예시 기준값, 필요에 맞게 조정 가능)
  const internetBasePrice = 23100; 

  // 선택된 TV 가격 (선택 안 되었으면 0원)
  const currentTvPrice = tvOptions.find(t => t.id === selectedTv)?.price || 0;
  
  // 총 예상 요금 (인터넷 + 선택된 TV)
  const totalEstimatedPrice = internetBasePrice + currentTvPrice;
  ...
  return (
    <div>
      <section className="container benefit">
        ...
        {/* TV 선택 섹션 */}
        <div className="mb-10">
          <div className="text-black text-xl lg:text-2xl font-bold mb-4">TV</div>
            ...
            {tvOptions.map((tv) => {
              ...
              return (
                <div
                  key={tv.id}
                  onClick={() => {
                    // 이미 선택된 걸 누르면 해제(null), 안 된 걸 누르면 선택
                ...
                >
                  <div>
                    <div className="text-blue-600 text-sm font-bold">{tv.title}</div>
        ...
        {/* 하단 예상 금액 결과 박스 */}
        <div className="bg-blue-600 rounded-[14px] p-6 lg:p-8 text-white flex flex-col md:flex-row items-center justify-between shadow-lg my-8">
          <div className="mb-4 md:mb-0">
            <div className="text-lg lg:text-xl font-bold">매월 납부하실 예상 금액</div>
            <div className="text-blue-100 text-xs lg:text-sm mt-1">
              {selectedTv ? `선택하신 인터넷과 ${tvOptions.find(t => t.id === selectedTv)?.title} TV 채널의 요금 합계입니다.` : "인터넷 단독 요금 기준입니다. (TV를 선택하시면 합산됩니다)"}
            </div>
          </div>
          <div className="text-right">
            <span className="text-sm lg:text-base font-medium mr-2">예상 월 요금:</span>
            <span className="text-3xl lg:text-4xl font-extrabold text-yellow-300">
              {totalEstimatedPrice.toLocaleString()}원
            </span>
            <div className="text-blue-100 text-xs mt-1">3년 약정 기준 / VAT 포함</div>
          </div>
        </div>
      </section>
