...
export default function SkPlan() {
  // TV는 기본적으로 아무것도 선택되지 않도록 빈 값("")으로 설정하여 초기 계산 및 테두리 제외
  const [selectedTv, setSelectedTv] = useState("");

  // TV 옵션 데이터
  const tvOptions = [
    { id: "economy", title: "이코노미 TV", sub: "절약형 236채널", price: 13200 },
    { id: "standard", title: "스탠다드 TV", sub: "실속형 236채널", price: 16500 },
    { id: "all", title: "ALL TV", sub: "고급형 252채널", price: 19800 },
  ];

  // 인터넷 기본 요금 (100M, 500M, 1G)
  const internetBasePrices = [23100, 34100, 39600];
  const internetDiscPrices = [18700, 23100, 31900];

  // 선택된 TV의 추가 요금 (선택 안 되었으면 0원)
  ...
  return (
    <div>
      <section className="container benefit">
        ...
        {/* 1. 인터넷 + WIFI 요금표 (두 번째 이미지의 원본 요금 안내 형식 유지) */}
        <div className="w-full overflow-x-auto lg:my-10 my-6">
          <div className="min-w-[767px] mx-auto rounded-[10px] overflow-hidden shadow-[0_4px_24px_0_rgba(0,0,0,0.08)]">
            <div className="bg-[#0f382b] border-t border-x border-stone-300 flex items-center justify-center h-10 lg:h-16 relative">
              <span className="text-white text-base lg:text-xl font-bold">인터넷 + WIFI {selectedTv ? `+ ${tvOptions.find(t => t.id === selectedTv)?.title}` : ''}</span>
            </div>
            ...
            {/* 요금 영역 (선택된 TV 가격이 있을 경우에만 자동 가산) */}
            <div className="grid grid-cols-3 border border-stone-300 !border-t-0">
              {internetBasePrices.map((p, i) => {
                const calculatedPrice = p + currentTvPrice;
                return (
                  <div key={i} className={`flex items-center justify-center bg-white py-4 ${i!==0 ? 'border-l border-stone-300' : ''}`}>
                    <span className="text-black text-sm lg:text-xl font-bold">월 {calculatedPrice.toLocaleString()}원</span>
                  </div>
                );
              })}
            </div>

            {/* 할인 영역 (선택된 TV 가격이 있을 경우에만 자동 가산) */}
            <div className="grid grid-cols-3 border border-stone-300 !border-t-0">
              {internetDiscPrices.map((p, i) => {
                const calculatedDiscPrice = p + currentTvPrice;
                return (
                  <div key={i} className={`flex items-center gap-2 justify-center bg-white py-4 ${i!==0 ? 'border-l border-stone-300' : ''}`}>
                    ...
                    <span className="text-[#0f382b] text-base lg:text-2xl font-extrabold">{calculatedDiscPrice.toLocaleString()}원</span>
                  </div>
                );
              })}
            </div>
      ...
      <section className="container py-10 lg:py-20">
        ...
        {/* TV 선택 카드 영역 (첫 화면에서 클릭되어 있지 않도록 설정) */}
        <div className="mt-8 mb-4">
          <div className="text-black text-xl lg:text-2xl font-bold mb-4">TV 선택 (선택사항)</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {tvOptions.map((tv) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="text-[#0f382b] text-sm font-bold">{tv.sub.split(" ")[0]}</div>
                    <div className="text-black text-xl lg:text-2xl font-extrabold mt-1">{tv.sub.split(" ")[1]}</div>
                    <div className="text-stone-500 text-sm font-medium mt-1">{tv.title}</div>
                  </div>
